...
class MasterCompilerApp:
    ...
    def ingest_planets(self):
        try:
            parsed = clean_and_parse_planets(pyperclip.paste())
            self.raw_parsed_data["planets"] = parsed.get("planetary_positions", [])
    ...
    def ingest_avasthas(self):
        try:
            self.raw_parsed_data["avasthas"] = clean_and_parse_avastha(pyperclip.paste()).get("avasthas", [])
    ...
    def launch_aspects_ui(self):
        ...
        def hijacked_export():
            ...
            if app.final_output:
                self.raw_parsed_data["aspects"] = app.final_output.get("aspect_strengths", {})
            top.destroy()
    ...
    def ingest_yogas(self):
        try:
            self.raw_parsed_data["yogas"] = clean_and_parse_yogas(pyperclip.paste()).get("yogas", {})
    ...
    def compile_and_save(self):
        ...
        if "planets" not in self.d1_master_data:
            self.d1_master_data["planets"] = {}

        for p_name in core_planet_names:
            if p_name in self.d1_master_data and p_name not in self.d1_master_data["planets"]:
                self.d1_master_data["planets"][p_name] = self.d1_master_data.pop(p_name)

        final_payload = {
            "planets": copy.deepcopy(self.d1_master_data.get("planets", {})),
            "signs": copy.deepcopy(self.d1_master_data.get("signs", {}))
        }

        lagna_sign = "aries"
        for p in self.raw_parsed_data.get("planets", []):
        ...
        def get_sign_for_house(house_num):
            idx = (lagna_idx + (house_num - 1)) % 12
            return ZODIAC_ORDER[idx]

        for i, sign in enumerate(ZODIAC_ORDER):
            if sign not in final_payload["signs"]: final_payload["signs"][sign] = {}
            h_num = (i - lagna_idx) % 12 + 1
            final_payload["signs"][sign][f"{self.varga_name}_house_number"] = h_num
            final_payload["signs"][sign][f"{self.varga_name}_house_lord"] = RASI_LORDS[sign].lower()
            final_payload["signs"][sign][f"{self.varga_name}_occupants"] = []

        for p_data in self.raw_parsed_data.get("planets", []):
            name = p_data.get("body", "").lower()
            sign = p_data.get("rasi", "").lower()
            if not sign: continue

            if name in core_planet_names:
                final_payload["signs"][sign][f"{self.varga_name}_occupants"].append(name)
                if name not in final_payload["planets"]: final_payload["planets"][name] = {}

                varga_block = {
                    f"{self.varga_name}_placement": {
                ...
                if p_data.get("special_dignity"):
                    varga_block[f"{self.varga_name}_placement"]["special_dignity"] = p_data.get("special_dignity")
                if p_data.get("upachaya_effect"):
                    varga_block[f"{self.varga_name}_placement"]["upachaya_effect"] = p_data.get("upachaya_effect")
                if p_data.get("is_retrograde"):
                    varga_block[f"{self.varga_name}_placement"]["is_retrograde"] = True

                deep_merge(final_payload["planets"][name], varga_block)
            elif name != "lagna":
                if f"{self.varga_name}_special_points" not in final_payload["signs"][sign]:
                    final_payload["signs"][sign][f"{self.varga_name}_special_points"] = []
                ...
                final_payload["signs"][sign][f"{self.varga_name}_special_points"].append(sp_payload)
        ...
        for sign, sign_data in final_payload["signs"].items():
            special_points = sign_data.get(f"{self.varga_name}_special_points", [])
            occupants = sign_data.get(f"{self.varga_name}_occupants", [])

            if special_points:
                occ_longs = {}
                for occ in occupants:
                    varga_block = final_payload["planets"].get(occ, {}).get(f"{self.varga_name}_placement", {})
                ...
                for sp in special_points:
                    ...
                    if deg is not None: sp_longs[sp["name"]] = deg

                # Execute Math logic before string truncation
                for sp in special_points:
                    sp_name = sp["name"]
        ...
        for av in self.raw_parsed_data.get("avasthas", []):
            name = av.get("body", "").lower()
            if name in core_planet_names and name in final_payload["planets"]:
                final_payload["planets"][name][f"{self.varga_name}_avastha_alertness"] = av.get("alertness")
                if av.get("age"):
                    final_payload["planets"][name][f"{self.varga_name}_avastha_age"] = av.get("age")
                if av.get("moods"):
                    final_payload["planets"][name][f"{self.varga_name}_avastha_moods"] = av.get("moods")

        p2p_aspects = self.raw_parsed_data.get("aspects", {}).get("planet_to_planet_aspects", {})
        for receiver, casters in p2p_aspects.items():
            name = receiver.replace("_receives_aspects", "").lower()
            if name in core_planet_names and name in final_payload["planets"]:
                final_payload["planets"][name][f"{self.varga_name}_aspects_received"] = casters

        asc_aspects = self.raw_parsed_data.get("aspects", {}).get("from_ascendant_to_houses", {})
        for key, casters in asc_aspects.items():
            parts = key.split("_")
            if len(parts) >= 2 and parts[0] == "house":
                h_num = int(parts[1])
                sign_name = parts[2].lower() if len(parts) >= 3 else get_sign_for_house(h_num)
                if sign_name in final_payload["signs"]:
                    final_payload["signs"][sign_name][f"{self.varga_name}_aspects_received"] = casters

        sav = self.raw_parsed_data.get("ashtakavarga", {}).get("samudayashtakavarga", {})
        bav = self.raw_parsed_data.get("ashtakavarga", {}).get("bhinnashtakavarga", {})
        for sign in ZODIAC_ORDER:
            if sign in sav:
                final_payload["signs"][sign][f"{self.varga_name}_sav"] = sav[sign]
                final_payload["signs"][sign][f"{self.varga_name}_bav"] = {}
                for p in ["sun", "moon", "mars", "mercury", "jupiter", "venus", "saturn", "lagna"]:
                    if p in bav and sign in bav[p]:
                        final_payload["signs"][sign][f"{self.varga_name}_bav"][p] = bav[p][sign]

        def evaluate_planet_strength(p_name):
            if p_name not in final_payload["planets"]: return 100, ""

            placement = final_payload["planets"][p_name].get(f"{self.varga_name}_placement", {})
            ...
            avastha = final_payload["planets"][p_name].get(f"{self.varga_name}_avastha_alertness", "").lower()
            ...
            dispositor = final_payload["signs"].get(sign, {}).get(f"{self.varga_name}_house_lord")
            if dispositor and dispositor in final_payload["planets"]:
                disp_placement = final_payload["planets"][dispositor].get(f"{self.varga_name}_placement", {})
            ...
            bav = final_payload["signs"].get(sign, {}).get(f"{self.varga_name}_bav", {}).get(p_name)
            sav = final_payload["signs"].get(sign, {}).get(f"{self.varga_name}_sav")
            if bav is not None:
                if bav >= 5: score += 10; reasons.append(f"High BAV {bav} (+10)")
                elif bav <= 2: score -= 10; reasons.append(f"Low BAV {bav} (-10)")
            if sav is not None:
                if sav >= 30: score += 10; reasons.append(f"High SAV {sav} (+10)")
                elif sav < 25: score -= 10; reasons.append(f"Low SAV {sav} (-10)")

            # 6. Nodal & Shadow Point Afflictions (With Upachaya Resilience check)
            sign_data = final_payload["signs"].get(sign, {})
            occupants = sign_data.get(f"{self.varga_name}_occupants", [])
            for node in ["rahu", "ketu"]:
                if node in occupants and node != p_name:
                    node_placement = final_payload["planets"].get(node, {}).get(f"{self.varga_name}_placement", {})
            ...
            special_pts = sign_data.get(f"{self.varga_name}_special_points", [])
            ...
            # 7. Aspects Received (Drishti)
            aspects = final_payload["planets"][p_name].get(f"{self.varga_name}_aspects_received", {})
            ...
            for caster, data in aspects.items():
                ...
                if strength >= 60:
                    if caster in benefics:
                        ...
                        caster_placement = final_payload["planets"].get(caster, {}).get(f"{self.varga_name}_placement", {})
        ...
        planet_evaluations = {}
        for p_name in core_planet_names:
            planet_evaluations[p_name] = evaluate_planet_strength(p_name)

        giver_map = {"Su": "sun", "Mo": "moon", "Ma": "mars", "Me": "mercury", "Ju": "jupiter", "Ve": "venus", "Sa": "saturn", "Ra": "rahu", "Ke": "ketu", "Md": "maandi"}
        for yoga_key, yoga_data in self.raw_parsed_data.get("yogas", {}).items():
            givers_str = yoga_data.get("yoga_givers", "")
            involved_planets = []

            for abbr, full_name in giver_map.items():
                if full_name in final_payload["planets"] and re.search(rf'\b{abbr}\b', givers_str):
            ...
            if involved_planets:
                ...
                if avg_score >= 70: status = "Active"
                elif avg_score >= 40: status = "Dormant"
                else: status = "Asleep"
                ...
                for p in involved_planets:
                    if f"{self.varga_name}_yogas" not in final_payload["planets"][p]:
                        final_payload["planets"][p][f"{self.varga_name}_yogas"] = {}
                    final_payload["planets"][p][f"{self.varga_name}_yogas"][yoga_key] = yoga_payload

        argala_data = self.raw_parsed_data.get("argala", {})
        for house_str, data in argala_data.items():
            h_num = int(house_str.split("_")[1])
            sign_name = get_sign_for_house(h_num)
            if sign_name in final_payload["signs"]:
                working = data.get("working_argala", {})
                if working: final_payload["signs"][sign_name][f"{self.varga_name}_effective_argala"] = working

        arudhas_data = self.raw_parsed_data.get("arudhas", {})
        for category, mappings in arudhas_data.items():
            for label, h_num in mappings.items():
                sign_name = get_sign_for_house(h_num)
                cat_key = f"{self.varga_name}_{category}"
                if sign_name in final_payload["signs"]:
                    if cat_key not in final_payload["signs"][sign_name]:
                        final_payload["signs"][sign_name][cat_key] = []
                    final_payload["signs"][sign_name][cat_key].append(label.upper())

        for sign in ZODIAC_ORDER:
            if not final_payload["signs"][sign].get(f"{self.varga_name}_occupants"):
                final_payload["signs"][sign].pop(f"{self.varga_name}_occupants", None)
