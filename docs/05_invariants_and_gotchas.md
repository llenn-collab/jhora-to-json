...
**G-10 — Aspect value types differ between modes (resolved at capture, 2026-09-15, F-01).**
The *parsers* still store different types: paste mode (`clean_and_parse_aspects`) stores **bare floats**; manual mode stores **`{"strength": float, "relation": str}` objects**. `compile_and_save`'s scoring reads `data.get("strength", 0)`. The crash path this created is closed by `normalize_aspect_values` in `compile_all.py`, which converts paste-mode floats to the object shape (`relation: "neutral"`) in `launch_aspects_ui`'s capture closure **before** anything reaches compile — the merged master JSON therefore only ever contains object values. Do not reintroduce raw float capture: `normalize_aspect_values` must stay on the aspects capture path, and the standalone paste parser must keep emitting bare floats (frozen contract).
